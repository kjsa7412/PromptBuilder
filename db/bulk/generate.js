...
function generateSQL(workFile, data) {
  ...
  lines.push('        id, user_id, title, description, tags, visibility, body_markdown,');
  lines.push('        clip_count, like_count, generate_count, view_count, is_deleted,');
  ...
  if (post_prompts.length > 0) {
    ...
    post_prompts.forEach((pp, i) => {
      ...
      lines.push(`        id, prompt_id, title, template_body, sort_order, status,`);
      lines.push(`        created_at, updated_at`);
      lines.push(`    ) VALUES (`);
      lines.push(`        gen_random_uuid(),`);
      ...
      if (variables.length > 0) {
        ...
        variables.forEach((v, j) => {
          ...
          lines.push(`        id, post_prompt_id, key, label, description, type, sort_order, created_at`);
          lines.push(`    ) VALUES (`);
          lines.push(`        gen_random_uuid(),`);
  ...
  lines.push(`    RAISE NOTICE 'INSERT OK: ${title.replace(/'/g, "''")} (prompt_id=%)', v_prompt_id;`);
  lines.push('END $$;');
  lines.push('');
...
function main() {
  if (ADMIN_USER_ID === 'YOUR_ADMIN_USER_ID_HERE') {
    ...
    console.error('   또는 generate.js 상단의 ADMIN_USER_ID 상수를 직접 수정하세요.');
    process.exit(1);
  }
  ...
  for (const filename of workFiles) {
    ...
    try {
      ...
      if (!data.post_prompts || data.post_prompts.length === 0) {
        throw new Error('post_prompts 배열이 비어있습니다');
      }

      const sql = generateSQL(filename, data);
  ...
  if (successCount > 0) {
    console.log('\n📋 다음 단계:');
    console.log('   sql/ 폴더의 .sql 파일을 Supabase SQL Editor에서 실행하세요.');
