...
import HomeTagFeed from '@/components/HomeTagFeed';

const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8080';

async function getTotalPrompts(): Promise<number> {
  try {
    const res = await fetch(`${API_BASE}/api/public/stats`, { cache: 'no-store' });
    const json = await res.json();
    return json?.data?.totalPrompts ?? 0;
  } catch {
    return 0;
  }
}
...
export default async function HomePage() {
  const totalPrompts = await getTotalPrompts();
